                ...
                InsertionMode::InHead => {
                    match token {
                        ...
                        Some(HtmlToken::StartTag {
                            ...
                            // 仕様書には定められていないが、このブラウザは仕様すべてを
                            // 実装しているわけではないので、<head>が省略されている
                            // HTML文書を扱うために必要。これがないと<head>が省略されている
                            // HTML文書で無限ループが発生する
                            if tag == "body" {
...
mod tests {
    ...
    fn test_body() {
        ...
        let html = document
            .borrow()
            .first_child()
            .expect("failed to get a first of document");
        assert_eq!(
        ...
        let head = html
            .borrow()
            .first_child()
            .expect("failed to get a first of html");
        assert_eq!(
            Rc::new(RefCell::new(Node::new(NodeKind::Element(Element::new(
                "head",
                Vec::new()
